...
def create_blockchain(block, nonce, last_block, merkle_root):
    ...
    if block_number % 2 == 1:
         ...
         sendTransaction(block_chain,20000)
...
while True:
    ...
    # If Tx is client A then call TTT instruction to update the temp file
    if Tx[3] == 'client_A': # Request from client A
        TTT_instructions(Tx)
    # If Tx is client B then also call TTT instructions to update the temp file
    elif Tx[3] == 'client_B':
        TTT_instructions(Tx)
